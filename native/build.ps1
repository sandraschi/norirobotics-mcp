...
if (Test-Path $specFile) {
    ...
    $testPort = 11999
    $oldHost = $env:NORI_MCP_HOST; $oldPort = $env:NORI_MCP_PORT
    $env:NORI_MCP_HOST = "127.0.0.1"; $env:NORI_MCP_PORT = "$testPort"
    $testProc = Start-Process -FilePath $frozenExe -ArgumentList "--serve" -NoNewWindow -PassThru -RedirectStandardError "$Root\dist\pyi-crash.log"
    Start-Sleep -Seconds 5
    $env:NORI_MCP_HOST = $oldHost; $env:NORI_MCP_PORT = $oldPort
