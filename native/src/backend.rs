...
const BACKEND_NAME: &str = "norirobotics-mcp-backend.exe";
const BACKEND_PORT: u16 = 11970;
...
/// enough -- orphaned child processes can hold the port even after the
...
fn dev_backend_path() -> Option<PathBuf> {
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("norirobotics-mcp-backend-x86_64-pc-windows-msvc.exe");
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    // norirobotics_mcp's __main__.py has no --port/--host CLI flags — host/port
    // come from load_settings() (pydantic-settings, env_prefix "NORI_MCP_").
    // So configuration goes through env vars, not argv, unlike the resonite-mcp
    // template this file was adapted from.
    ...
    // (uvicorn's default logging config writes to stderr, so check both
    // streams rather than stdout only).
    ...
    if let Some(stderr) = child.stderr.take() {
        thread::spawn(move || {
            for line in BufReader::new(stderr).lines().map_while(Result::ok) {
                ...
                if line.contains("Uvicorn running") || line.contains("Application startup complete") {
                    let _ = app_log_stderr.emit("backend-status", "ready");
                }
            }
        });
    }
    ...
    Ok("Backend starting on port 11970".into())
}
